...
export function SectionHeader({ title, subtitle, right }: { title: string; subtitle?: string; right?: React.ReactNode }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ minWidth: 0, flex: isMobile ? '0 0 100%' : '1 1 0%' }}>
      ...
      {right && <div style={{ flex: isMobile ? '0 0 100%' : '1 1 auto', minWidth: 0, display: 'flex', justifyContent: isMobile ? 'flex-start' : 'flex-end' }}>{right}</div>}
    </div>
  );
}