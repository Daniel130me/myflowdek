import { z } from 'zod';
import { DEFAULT_CURRENCY, isPaymentSupportedCurrency, PAYMENT_SUPPORTED_CURRENCIES } from '@/lib/currency';
...
export const createEngagementSchema = z.object({
  ...
  // Default from the single currency constant (NGN at launch) and validate
  // up front: a non-fundable currency would only fail later, at fund time.
  currency: z
    .string()
    .regex(currencyRegex, 'Currency must be a 3-letter ISO code')
    .refine(isPaymentSupportedCurrency, {
      message: `Engagements currently support ${PAYMENT_SUPPORTED_CURRENCIES.join('/')} payments only at launch`,
    })
