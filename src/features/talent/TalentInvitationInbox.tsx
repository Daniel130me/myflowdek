...
export function TalentInvitationInbox() {
  ...
  return <main className={styles.surface}>
    ...
    {loading ? <div className={styles.loading}>Loading invitations…</div> : invitations.length === 0 ? <section className={styles.emptyDirectory}><Inbox size={28} /><h2>No invitations yet</h2><p>Task invitations sent to your professional profile will appear here.</p></section> : <section className={styles.inboxList} aria-label="Professional invitations">{invitations.map((invitation) => <article className={styles.inboxCard} key={invitation.id}><div className={styles.inboxCardTop}><div><span className={styles.statusLabel}>{humanizeTalentEnum(invitation.status)}</span><h2>{invitation.task.name}</h2><p>Invited by {invitation.invitedBy.name ?? 'a Flowdek manager'} · {formatDate(new Date(invitation.createdAt))}</p></div>{invitation.proposedBudget && <strong>{invitation.currency} {formatNumber(Number(invitation.proposedBudget))}</strong>}</div>{invitation.message && <p className={styles.invitationMessage}>{invitation.message}</p>}<dl className={styles.invitationFacts}><div><dt>Proposed deadline</dt><dd>{invitation.proposedDeadline ? formatDate(new Date(invitation.proposedDeadline)) : 'Not specified'}</dd></div><div><dt>Invitation expires</dt><dd>{formatDate(new Date(invitation.expiresAt))}</dd></div></dl>{invitation.status === 'PENDING' && <div className={styles.inboxActions}><button className={styles.secondaryButton} type="button" disabled={pendingId === invitation.id} onClick={() => void respond(invitation.id, 'decline')}>Decline</button><button className={styles.primaryButton} type="button" disabled={pendingId === invitation.id} onClick={() => void respond(invitation.id, 'accept')}>I’m interested</button></div>}</article>)}</section>}
  </main>;
}
