...
/**
 * Keyboard cheat sheet (audit H-23). Rendered through the Radix-backed
 * Modal primitive — Escape genuinely closes the dialog (the promise the
 * cheat sheet itself makes), focus is trapped while open and restored on
 * close, background scroll locks, and screen readers announce a dialog.
 */
export function KeyboardShortcutsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  if (isMobile) {
    return (
      <Modal open={open} onClose={onClose} label="Keyboard shortcuts" variant="bottom-sheet" zIndex={50}>
  ...
  return (
    <Modal
      ...
      variant="center"
      zIndex={50}
      style={{ width: 'min(520px, 92vw)', maxHeight: '85vh', padding: 24 }}
    >
      {/* Header */}
