...
export function CloudFilePickerModal({
  ...
  const handleAttach = async (fileItem: ProviderFileItem) => {
    ...
    try {
      ...
      toast.success(`Attached "${fileItem.name}" from ${PROVIDER_NAMES[activeProvider]}`);
      onFileAttached();
      onClose();
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: COLORS.gray }}>
                Browse and link files directly from your cloud drive (no copy stored in Flowdek)
              </div>
            </div>
          </div>
          <button
        ...
        {/* Toolbar: Provider Selector & Search */}
        <div
          style={{
            padding: '12px 20px',
            background: '#F9FAFB',
        ...
        {connected && accountEmail && (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span>Connected as <strong>{accountEmail}</strong></span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <button
                onClick={() => fetchProviderFiles(activeProvider, query)}
                style={{ border: 'none', background: 'none', cursor: 'pointer', color: '#059669', display: 'flex', alignItems: 'center', gap: 4, fontSize: 12 }}
              >
                <RefreshCw size={12} /> Refresh
              </button>
              <a
                href="/settings"
                style={{ color: '#059669', textDecoration: 'underline', fontSize: 12, fontWeight: 500 }}
              >
                Manage Drives
              </a>
            </div>
          ...
          {!connected && (
            <div style={{ textAlign: 'center', padding: '36px 20px' }}>
              ...
              <div style={{ fontSize: 13, color: COLORS.gray, maxWidth: 400, margin: '0 auto 16px' }}>
                Flowdek does not upload or copy your files. Authorize access to attach and open files directly from your personal drive.
              </div>
