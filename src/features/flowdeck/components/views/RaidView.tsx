...
import { COLORS, RAID_META, RAID_ORDER, IMPACT_META, TEAM, teamById, TODAY, type RaidItem } from '@/features/flowdeck/model';
import { Avatar, SectionHeader, selectStyle, FF } from '../ui';
import { useViewport } from '../../hooks/useViewport';

export function RaidView({ items, onAdd, onUpdate, onRemove }: { items: RaidItem[]; onAdd: (item: RaidItem) => void; onUpdate: (id: string, patch: Partial<RaidItem>) => void; onRemove: (id: string) => void }) {
  const { isMobile } = useViewport();
  const [filter, setFilter] = useState('all');
  const [showForm, setShowForm] = useState(false);
  const [draft, setDraft] = useState({ type: 'risk', description: '', owner: TEAM[0].id, impact: 'medium' });
  ...
  function submit() {
    ...
    setDraft({ type: 'risk', description: '', owner: TEAM[0].id, impact: 'medium' });
    setShowForm(false);
  }
        ...
        {showForm && (
          <div style={{ background: COLORS.card, border: `1px solid ${COLORS.line}`, borderRadius: 12, padding: 16, marginBottom: 12 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
              ...
              <select value={draft.owner} onChange={e => setDraft({ ...draft, owner: e.target.value })} style={selectStyle}>{TEAM.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select>
            </div>
              ...
              <div key={item.id} style={{ background: COLORS.card, border: `1px solid ${COLORS.line}`, borderRadius: 12, padding: '14px 16px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    <span style={{ fontSize: 12.5, color: COLORS.gray, fontFamily: FF }}>{teamById[item.owner]?.name.split(' ')[0]}</span>
                  </div>
          ...
          {shown.map(item => {
            ...
            return (
              <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '110px 2.2fr 1fr 100px 1fr 36px', alignItems: 'center', padding: '11px 16px', borderBottom: `1px solid ${COLORS.line}`, fontSize: 13 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Avatar id={item.owner} size={20} /> <span style={{ fontSize: 12.5 }}>{teamById[item.owner]?.name.split(' ')[0]}</span></div>
